...
do
  ip_suffix=`printf "%x\n" $i`
  ip_addr=" 2001:db8:132${ip_suffix}::1/64"  > /dev/null
  ...
  if [ $? -eq 0 ]; then
    ...
    echo "ip addr add 2001:db8:132::1/64 dev azumi" >> /etc/private.sh
        ip_addr=" 2001:db8:13${ip_suffix}::1/64"
...
display_notification $'\e[93mAdding cron job for server!\e[0m'
	...
	ping -c 2 2001:db8:132::2 | sed "s/.*/\x1b[94m&\x1b[0m/" 
	sleep 1
...
do
    ip_suffix=`printf "%x\n" $i`
    ip_addr=" 2001:db8:13${ip_suffix}::1"
    echo "+---------------------------+"
...
do
  ip_suffix=`printf "%x\n" $i`
  ip_addr=" 2001:db8:13${ip_suffix}::2/64" > /dev/null
...
# private.sh
    ...
    echo "ip tunnel add azumi mode sit remote $remote_ip local $local_ip ttl 255" >> /etc/private.sh
    echo "ip link set dev azumi up" >> /etc/private.sh
    echo "ip addr add 2001:db8:132::2/64 dev azumi" >> /etc/private.sh
        ip_addr=" 2001:db8:13${ip_suffix}::2/64"
        echo "ip addr add $ip_addr dev azumi" >> /etc/private.sh
	...
	ping -c 2 2001:db8:132::1 | sed "s/.*/\x1b[94m&\x1b[0m/" 
	sleep 1
	display_notification $'\e[93mConfiguring keepalive service..\e[0m'
...
do
    ip_suffix=`printf "%x\n" $i`
    ip_addr=" 2001:db8:13${ip_suffix}::2"
    echo "+---------------------------+"
    echo -e "| \e[92m$ip_addr    \e[0m|"
done
echo "+---------------------------+"
